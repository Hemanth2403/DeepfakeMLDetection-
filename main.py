...
try:
    ...
    # APEX may not be available on Windows or in CPU-only environments.
    # Provide a lightweight fallback so the code can run without APEX.
    class _ApexFallback:
        @staticmethod
        def initialize(model, optimizer, **kwargs):
            # Return model and optimizer unchanged
